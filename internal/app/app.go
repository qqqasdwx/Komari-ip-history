...
func Run() error {
	...
	api := router.Group(cfg.APIBase())
	{
		...
		admin.Use(middleware.RequireAdmin())
		{
			...
			admin.GET("/notifications/settings", adminHandler.GetNotificationSettings)
			admin.PUT("/notifications/settings", adminHandler.PutNotificationSettings)
			admin.GET("/notifications/channels", adminHandler.ListNotificationChannels)
			admin.POST("/notifications/channels", adminHandler.CreateNotificationChannel)
			admin.PATCH("/notifications/channels/:id", adminHandler.UpdateNotificationChannel)
			admin.DELETE("/notifications/channels/:id", adminHandler.DeleteNotificationChannel)
			admin.POST("/notifications/channels/:id/test", adminHandler.TestNotificationChannel)
			admin.GET("/notifications/rules", adminHandler.ListNotificationRules)
			admin.POST("/notifications/rules", adminHandler.CreateNotificationRule)
			admin.PATCH("/notifications/rules/:id", adminHandler.UpdateNotificationRule)
			admin.DELETE("/notifications/rules/:id", adminHandler.DeleteNotificationRule)
			admin.GET("/notifications/logs", adminHandler.ListNotificationDeliveryLogs)
			admin.DELETE("/notifications/logs", adminHandler.ClearNotificationDeliveryLogs)
