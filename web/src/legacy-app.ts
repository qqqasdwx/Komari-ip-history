...
function appLayout(active: AppSection, content: string) {
  return `
  ...
  return `
    <div class="page-head">
      <div class="page-head-main">
        ${
          options.backPath
            ? `<button class="back-link" data-back="${escapeHtml(options.backPath)}">${escapeHtml(
...
function renderNodes() {
  const rows = state.nodes
    .map(
      (item) => `
        <a class="node-list-row" href="#/nodes/${item.komari_node_uuid}">
          <div class="node-list-cell node-list-cell-name">
            <strong class="node-list-name">${escapeHtml(item.name)}</strong>
          </div>
          <div class="node-list-cell node-list-cell-status">
            <span class="status ${item.has_data ? "ok" : "empty"}">${item.has_data ? "有数据" : "无数据"}</span>
          </div>
          <div class="node-list-cell node-list-cell-updated muted">${formatDateTime(item.updated_at)}</div>
          <div class="node-list-cell node-list-cell-summary">
            ${renderNodeListSummary(item)}
          </div>
          <div class="node-list-cell node-list-cell-action">
            <span class="node-list-view">查看</span>
          </div>
        </a>`
    )
    .join("");
    ...
    `
      ...
      <section class="panel">
        <div class="section">
          ...
          <div class="node-list-table">${rows ? `
            ...
            <div class="node-list-body">${rows}</div>
          ` : `
            <div class="card empty-state-card">
              <h3>还没有节点</h3>
              <p class="muted">先复制 Header 到 Komari，然后在节点详情页点击“添加 IP 质量检测”。</p>
              <div class="toolbar">
                <button class="button" data-nav="/settings/integration">去接入</button>
              </div>
            </div>`}</div>
        </div>
      </section>
...
function highlightJavaScript(code: string) {
  const pattern =
    /(\/\/[^\n]*|\/\*[\s\S]*?\*\/|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`|\b(?:const|let|var|function|return|if|else|async|await|new|true|false|null|undefined|try|catch|throw)\b|\b(?:window|document|fetch|location|navigator|JSON|localStorage|sessionStorage)\b)/g;
  let lastIndex = 0;
  let highlighted = "";
  for (const match of code.matchAll(pattern)) {
    ...
    const token = match[0];
    let tokenClass = "code-token-neutral";
    if (token.startsWith("//") || token.startsWith("/*")) {
      tokenClass = "code-token-comment";
    } else if (token.startsWith('"') || token.startsWith("'") || token.startsWith("`")) {
      tokenClass = "code-token-string";
    } else if (
      /^(const|let|var|function|return|if|else|async|await|new|true|false|null|undefined|try|catch|throw)$/.test(token)
    ) {
      tokenClass = "code-token-keyword";
    } else {
      tokenClass = "code-token-builtin";
    }
...
function renderCodeBlock(code: string, compact = false) {
  return `<pre class="code-block${compact ? " code-block-compact" : ""}"><code class="code-content">${highlightJavaScript(code)}</code></pre>`;
}
...
function reportUsageText(value: unknown) {
  return reportUsageMeta(value).text;
}
...
function renderReportHeader(structured: ReturnType<typeof getFilteredCurrentResult>) {
...
function renderBaseInfoSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
  const info = structured?.info;
  if (!info) {
    return "";
  }
  const city = isRecord(info.City)
    ? [info.Region && isRecord(info.Region) ? info.Region.Name : "", info.City.Name, info.City.PostalCode && info.City.PostalCode !== "null" ? info.City.PostalCode : ""]
        .filter(Boolean)
        .join(", ")
    : "";
  const usagePlace = [
    ...
    .filter(Boolean)
    .join(", ");
  const registered = info.RegisteredRegion && isRecord(info.RegisteredRegion)
...
function renderTypeSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
  ...
  const usageMap = new Map((isRecord(type.Usage) ? orderedReportEntries(type.Usage, providerOrder) : []).map(([key, value]) => [reportFieldLabel(key), reportUsageMeta(value)]));
  const companyMap = new Map((isRecord(type.Company) ? orderedReportEntries(type.Company, providerOrder) : []).map(([key, value]) => [reportFieldLabel(key), reportUsageMeta(value)]));
  const columns = Array.from(new Set([...usageProviders, ...companyMap.keys()]));
  const rows = [
    {
      label: "使用类型",
      values: columns.map((column) => {
        const meta = usageMap.get(column);
        return meta ?? { text: "无", tone: "muted" };
      })
    },
    {
      label: "公司类型",
      values: columns.map((column) => {
        const meta = companyMap.get(column);
        return meta ?? { text: "无", tone: "muted" };
      })
    }
  ];
...
function renderScoreSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
...
function renderFactorSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
...
function renderMediaSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
...
function renderMailSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
...
function renderNodeListSummary(item: NodeListItem) {
  ...
  if (!item.has_data || !structured) {
    return `<div class="node-list-summary-inline muted" data-node-summary="empty">当前还没有检测结果</div>`;
  }
  ...
  return `
    <div class="node-list-summary-inline" data-node-summary="structured">
      ${scoreEntries.length > 0
        ? scoreEntries
            .map(
              (entry) => `
                <span class="node-list-pill">
                  <strong>${escapeHtml(entry.label)}</strong>
                  <span>${escapeHtml(entry.value)}</span>
                </span>
              `
            ...
            .join("")
        : `<span class="muted">当前没有可展示的风险分项</span>`}
    </div>
  `;
}
...
async function renderSettings(section: "integration" | "priority" | "admin") {
  ...
  const pageMap = {
        ...
        <section class="panel">
          <div class="section">
            ...
            <div class="card">
              ...
              ${renderCodeBlock(loaderPreview.code)}
            </div>
          </div>
          ...
          <div class="section">
            ...
            ${renderCodeBlock(inlinePreview.code, true)}
          </div>

          <div class="section">
            <div class="section-head">
              <h2>其他设置</h2>
              <span class="chip">已移到左侧菜单</span>
            </div>
            <div class="grid">
              <div class="card">
                <h3>变化规则</h3>
                ...
                <div class="toolbar">
                  <button class="button ghost" data-nav="/settings/priority">去设置</button>
                </div>
              </div>
              <div class="card">
                <h3>管理员设置</h3>
                <p class="muted">修改登录用户名和密码。</p>
                <div class="toolbar">
                  <button class="button ghost" data-nav="/settings/admin">去设置</button>
                </div>
              </div>
            </div>
          </div>
    ...
    priority: {
      title: "变化优先级规则",
      subtitle: "控制哪些变化算重点、哪些算辅助",
      content: `
        <section class="panel">
          <div class="grid">
            <div class="card">
              <h3>规则目标</h3>
              <div class="metric-value">${priorityTargets.length}</div>
              <p class="muted">当前可配置的分组数量。</p>
            </div>
            <div class="card">
              <h3>辅助变化</h3>
              <div class="metric-value">${secondaryCount}</div>
              <p class="muted">已标记为辅助变化的路径数量。</p>
            </div>
          </div>
          <div class="section">
            <h2>变化优先级规则</h2>
      ...
      content: `
        <section class="panel">
          <div class="summary-section">
            <div class="summary-head">
              <strong>当前管理员</strong>
              <span class="chip">单管理员模式</span>
            </div>
            <div class="muted">当前登录账号：${escapeHtml(state.me?.username ?? "admin")}。修改后会要求重新登录。</div>
          </div>
          <div class="section">
            <h2>管理员设置</h2>
