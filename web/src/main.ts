...
function reportRiskMeta(value: unknown) {
  ...
  if (numeric <= 5) {
    return { score: text, label: "极低风险", tone: "good", percent };
  }
  if (numeric <= 25) {
    return { score: text, label: "低风险", tone: "good", percent };
  }
  if (numeric <= 60) {
    return { score: text, label: "中风险", tone: "warn", percent };
  }
  if (numeric <= 85) {
    return { score: text, label: "高风险", tone: "bad", percent };
  }
  return { score: text, label: "极高风险", tone: "bad", percent };
}
...
function renderMailSection(structured: ReturnType<typeof getFilteredCurrentResult>) {
  ...
  const comms = providers
    ...
    .map((provider) => {
      ...
      return `<span class="report-mail-chip ${ok ? "good" : "bad"}">${ok ? "+" : "-"}${escapeHtml(reportFieldLabel(provider))}</span>`;
    })
    .join("");
