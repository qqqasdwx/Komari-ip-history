# Komari-ip-history

`Komari-ip-history` 是一个独立部署的 IP 质量服务，用来把 `IPQuality` 的探测结果接到 `Komari` 节点里。

它的目标不是替代 `Komari`，而是补一条 IP 质量能力：

- 在不修改 `Komari` 官方源码的前提下，通过 `custom_head` 注入入口
- 在 `Komari` 节点详情页里直接打开 IP 质量结果
- 支持多 IP 目标、历史变化、快照对比、收藏快照、历史保留策略
- 支持独立节点、`Komari` 绑定、通知、API Key 和对外只读 API

## 这个项目适不适合你

如果你想要的是下面这些能力，这个项目是合适的：

- 在 `Komari` 节点详情页里查看当前 IP 质量
- 给一个节点配置多个目标 IP，分别采集和展示
- 查看某个 IP 的历史变化，而不只是当前一次结果
- 用快照对比页比较两个时间点的完整结果
- 收藏关键快照，避免被历史清理策略删除
- 独立部署 IP 质量服务，不改 `Komari` 官方源码

如果你只想看一次性的 `IPQuality` 结果，不需要：

- `Komari` 集成
- 历史变化
- 多 IP 管理

那这个项目会偏重。

## 当前功能

- `Komari` 节点详情页注入“打开 IP 质量”入口
- 管理员 / 游客分流
- 未接入节点点击后在新标签页打开 IPQ 后台，并自动定位到上报设置
- 节点列表行内“上报设置”弹窗
- 独立节点创建
- `Komari` 绑定 / 解绑
- 多 IP 目标管理
- 单目标 IP 启停
- 可配置 `cron` 周期
- `timezone` / `CRON_TZ`
- 安装后立即执行一次
- 单条接入命令，重跑即覆盖更新旧配置
- 自动探查计划接口
- 当前结果展示
- 历史变化列表
- 快照对比页
- 收藏快照
- 全局历史保留策略
- 通知通道 / 规则 / 测试发送 / 投递日志
- API Key 管理
- 对外只读 public API
- 节点内部稳定 `node_uuid`（兼容旧 `komari_node_uuid`）

## 当前已知限制（截至 2026-04-17）

- `IPQ Node + Komari Binding` 的底层主实体还没有完全拆表重构，当前虽已补内部 `node_uuid`，但仍在兼容旧 `Node` 模型
- public API 虽已补限流和访问日志，但长期稳定开放前仍建议继续观察错误语义与兼容边界
- 安装命令已改为当前服务端直出 install script，但正式发版前仍建议按具体版本回归验证一次节点侧安装流程

## 文档

- [集成与行为说明](docs/集成与行为说明.md)
- [用户操作手册](docs/用户操作手册.md)
- [API 草案 / 当前实现备注](docs/API草案-v1.md)
- [开发环境说明](docs/开发环境.md)
- [发版说明](docs/发版说明.md)

## 部署方式

当前只支持 Docker 部署。

镜像地址：

```bash
ghcr.io/qqqasdwx/komari-ip-history:latest
```

### 部署前准备

至少要准备这 2 个环境变量：

- `IPQ_DEFAULT_ADMIN_USERNAME`
  - 初始管理员用户名
- `IPQ_DEFAULT_ADMIN_PASSWORD`
  - 初始管理员密码

常用可选项：

- `IPQ_DB_PATH`
  - 镜像内默认是 `/data/ipq.db`
  - 只有你想改默认位置时才需要设置

### 用 Docker 直接启动

```bash
docker pull ghcr.io/qqqasdwx/komari-ip-history:latest

docker run -d \
  --name komari-ip-history \
  -p 8090:8090 \
  -v "$(pwd)/data:/data" \
  -e IPQ_DEFAULT_ADMIN_USERNAME=admin \
  -e IPQ_DEFAULT_ADMIN_PASSWORD='change-this-password' \
  # -e IPQ_DB_PATH=/data/ipq.db \
  ghcr.io/qqqasdwx/komari-ip-history:latest
```

启动后访问：

```text
http://your-server-ip:8090/#/login
```

### 用 Docker Compose 启动

先创建 `compose.yml`：

```yaml
services:
  ipq:
    image: ghcr.io/qqqasdwx/komari-ip-history:latest
    container_name: komari-ip-history
    restart: unless-stopped
    ports:
      - "8090:8090"
    environment:
      IPQ_DEFAULT_ADMIN_USERNAME: admin
      IPQ_DEFAULT_ADMIN_PASSWORD: change-this-password
      # IPQ_DB_PATH: /data/ipq.db
    volumes:
      - ./data:/data
```

启动命令：

```bash
docker compose up -d
```

更新命令：

```bash
docker compose pull
docker compose up -d
```

### 部署后怎么接 `Komari`

部署完成后，登录 IPQ 后台：

1. 打开 `接入配置`
2. 复制 `Komari custom_head` 用的注入代码
3. 粘贴到 `Komari` 的 `custom_head`
4. 回到 `Komari` 节点页，即可看到“打开 IP 质量”入口

当前未接入节点的行为是：

- 点击入口后，不再继续把后台配置流程嵌进 `Komari` 弹窗
- 而是在**新标签页**打开 IPQ 后台，并自动定位到对应节点的上报设置
- 配置完成后，用户再回到 `Komari` 查看当前结果

如果你想按界面一步步操作，可以直接看：

- [用户操作手册](docs/用户操作手册.md)

### 说明

- 当前默认按**根路径**部署
- 默认会按你当前访问服务时的地址动态生成：
  - 注入脚本地址
  - 接入命令里的服务端地址
  - 页面内跳转地址
- 如果前面有反向代理，请确保正确传递：
  - `Host`
  - `X-Forwarded-Proto`
- 真正上线时建议切 HTTPS

## 开发指南

开发环境、联调方式、种子数据、排障流程，放在单独文档里：

- [集成与行为说明](docs/集成与行为说明.md)
- [开发环境说明](docs/开发环境.md)
- [发版说明](docs/发版说明.md)

## 参考

- `Komari` 前端技术栈参考：`references/komari-web`
- `IPQuality` 字段和展示逻辑参考：`references/IPQuality`
